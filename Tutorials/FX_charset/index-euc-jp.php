...
header('Content-Type: text/html;charset=EUC-JP');	// Added by msyk 2004/3/23
...
$BookQuery->SetCharacterEncoding('EUC-JP');	// Added by msyk 2004/3/23
	// These lines pass the character set information to FX class.
$BookQuery->SetDataParamsEncoding('UTF-8');
	// When you use  EUC-JP for the character encoding, you need to specify
	// the data parameter encoding in spite of FMS7Adv.
	//    FMS7Adv -> 'UTF-8',  FM6 -> 'SJIS'  for Japanese.
...
//$BookQuery->SetDBData("Book_List.fp7", "Book_List.fp7");
...
if ($BookData['foundCount'] > 0) {					// Determine if any books were found.
	foreach ($BookData['data'] as $key => $value) {	// The 'data' subarray contains the search results from FileMaker
		...
		echo "<td align=\"left\" valign=\"top\">";
		echo "<a href=\"detail-euc-jp.php?ID=$recordID&query=" . urlencode($currentSearch) . "\">";
		echo $value['title'][0];
		echo "</a> [ ";
...
?>
			...
			<tr>
				<td colspan="1" align="left">
					<form action="index-euc-jp.php" method="post">
